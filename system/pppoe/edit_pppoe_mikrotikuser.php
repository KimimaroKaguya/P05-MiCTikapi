<?php
			...
			$user = $API->comm("/ppp/secret/print", array(
				...
				if(!empty($_REQUEST['username'])){
					...
					if($_REQUEST['username']==$_GET['id']){
	                   ...
	                   mysql_query("UPDATE pppoe_gen SET user='".$_REQUEST['username']."', pass='".$_REQUEST['password']."',  profile='".$_REQUEST['profile']."',  caller_id='".$_REQUEST['mac']."', comment='".$db_comment."' WHERE user='".$user."'");
    ...
    window.location.href = 'index.php?page=pppoe_mik_user';}})</script>";
						...
						if($num==0){
							...
							$db_comment=$_REQUEST['comment'];
						    $mt_comment=iconv("utf-8", "tis-620",$_REQUEST['comment']);
	                        mysql_query("UPDATE pppoe_gen SET user='".$_REQUEST['username']."', pass='".$_REQUEST['password']."',  profile='".$_REQUEST['profile']."',comment='".$db_comment."' WHERE user='".$user."'");
         ...
         <div class="col-lg-12" >
            <div class="<?php print panel_modify();?>">
					      ...
					      <option value="<?php echo $user['0']['profile']; ?>"><?php echo $user['0']['profile']; ?></option>
                            ...
                            <div class="col-xs-12  col-md-6">
                                <div class="form-group">
                                   ...
                                   <input name="mac" type="text" class="form-control" placeholder="Ex.172.0.0.3" value="<?php echo $user['0']['caller-id'];?>">  
							  </div>
                                </div>
                            </div>
						...
						<div class="row">
                            <div class="col-xs-12  col-md-6">
                                <div class="form-group">
                                   ...
                                   <input name="comment" type="comment" class="form-control" placeholder="Ex.jen/31/2017 23:00:00" value="<?php echo iconv("tis-620", "utf-8",$user['0']['comment']);?>" maxlength="30">  
							  </div>
                                </div>
                            <div class="col-xs-12  col-md-6">
                                <div class="form-group">
	          ...
	          ###ตรวจสอบโปรไฟล์ที่กำหนด expireและprice (วันหมดอายุและราคามีครบหรือยัง?)###
							$exp=mysql_fetch_array(mysql_query("SELECT * FROM pppoe_pro WHERE pro_name='".$user['0']['profile']."'"));
