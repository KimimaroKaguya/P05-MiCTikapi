...
$API = new routeros_api();
 ...
 #############################################################################################
 $sql="SELECT * FROM pppoe_money_year WHERE mt_id='".$id."'";

													$query=mysql_query($sql);	
													While($result=mysql_fetch_array($query)){	
     
    $thaiyear=($result['year']+543);
	$jan=$result['jan'];
...
$data= array($data_jan,$data_feb,$data_mar,$data_apr,$data_may,$data_jun,$data_jul,$data_aug,$data_sep,$data_oct,$data_nov,$data_december);
	...
	$series1['drilldown'][]=array('colorByPoint'=>true,'name'=>''.$thaiyear.'','id'=>$result['year'].'-id'.$id,'data'=>$data);

	}

################################################
  $sql="SELECT * FROM pppoe_money_month WHERE mt_id='".$id."'";
