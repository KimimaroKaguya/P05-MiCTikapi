			...
			$ARRAY = $API->comm("/ip/hotspot/user/profile/print");		
				...
				if(!empty($_REQUEST['username'])){
					...
					if($_REQUEST['username']==$_GET['id']){
						...
						mysql_query("UPDATE mt_gen SET user='".$_REQUEST['username']."', pass='".$_REQUEST['password']."', profile='".$_REQUEST['profile']."' WHERE user='".$user."'");
						...
						if($num==0){
							...
							@unlink("qrcode/".$img);
							QRcode::png('http://'.$ip.'/login?username='.$username.'&password='.$password.'', 'qrcode/'.$file.'');
							mysql_query("UPDATE mt_gen SET user='".$_REQUEST['username']."', pass='".$_REQUEST['password']."', profile='".$_REQUEST['profile']."',qrcode='".$file."' WHERE user='".$user."'");
		                        ...
		                        <div class="panel-body">
								   ...
								   ?>
                                        <div class="form-group input-group">
                                            ...
                                            <input type="text" name="username" placeholder="Username" class="form-control" value="<?php echo $result['user']; ?>" required>
                                        </div>
                                        <div class="form-group input-group">
                                            <span class="input-group-addon">Password&nbsp;</span>
                                            <input type="text" name="password" placeholder="Password" class="form-control" value="<?php echo $result['pass']; ?>" required>
                                        </div>
                                        <div class="form-group input-group">
